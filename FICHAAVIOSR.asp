        ...
        <td bgcolor="#ffffff" align="center">
            <table width="100%" border="1" bordercolordark="DarkOrange" cellpadding="3" cellspacing="0" bordercolorlight="#FFFFFF" > 
                <tr width="100%" height="50px">
	                ...
	                <font color="#000066" size="3" face="arial"><b>Registro de AVIOS</b></font> </td>
                </tr>
            </table>
		</td>	
...
<table  border="0" cellspacing="2" cellpadding="4" align="center" class="Estilo55" >
  ...
  <tr>
         ...
         <%tt1 = "RSFACCAR..ft"+cia+"ctae"
           cad =   " Select TC_CEXISTE,TC_CDESCRI From "&tt1&" WHERE TC_CEXISTE IN('241103','261103') Order by TC_CEXISTE    " 
             
            'response.write(cad) 
              RS.OPEN CAD, CNN           
          '  response.end
            IF RS.RECORDCOUNT > 0 THEN RS.MOVEFIRST %>
                <select  class="TEXTO" id="cta" name="cta"  value="" >
                    <option value=""></option>
                    <%DO WHILE NOT RS.EOF %>
                        <option value="<%=RS.FIELDS.ITEM(0)%>"
  ...
  <tr>
        ...
        value="<% if trim(rs2("ar_cmodelo"))= "" then response.write("AVIOS - RHIN AVIOS") else response.write(rs2("AR_CMODELO")+" Rhin" )%>"/></td>    
  </tr>
</table>
</td></tr>
<!--------------------------------------------------------------------->
...
// hay que validar que llene los campos obligatorios
    ...
    if (document.all.cta.value == '') {
        alert("Favor ingresar la Cuenta Contable");
        seleccionar(document.all.cta);
        return false;
    }
    cad = 'comun/crea_avios.asp'
