...
<!--#include file="includes/navegacion.inc"-->
<!--#include file="COMUN/FUNCIONESCOMUNES.ASP"-->
<%Dim pageSize 
if txtPerfil > 1 then 	pageSize = 16 else 	pageSize = 6

' CAPTURA EL SUBMENU PRINCIPAL SI NO ES UNA TABLA NORMAL... ES UN BROWSE PARA MAESTROS!
MEN = Request.QueryString("MENU")
IF MEN = "" OR ISNULL(MEN) OR MEN = " " THEN MEN = 1
' CAPTURA EL nombre del programa para mostrar la ficha SI NO ES UNA TABLA NORMAL... ES UN BROWSE PARA MAESTROS!
hlp = Request.QueryString("ficha")
IF hlp = "" OR ISNULL(hlp) OR hlp = " " THEN hlp = ""
...
<table width="100%" border="0" align="center">
	<tr>
		<td align= "left" width="25%"><img src="imagenes/ATRAS.gif" style="cursor:hand;" alt=""
			onclick="javascript: window.location.replace('SUBMENU.ASP?TIPO=<%=MEN%>');" /></td>
...
pag += '&pos='
perf = '<%=trim(txtPerfil)%>' 
brow = '<%=request.querystring("brow")%>' 
perf = parseInt(perf,10)
if (perf == 1)
	eval("DATAENTRY.style.display='block'");
if	(brow == '1')
	eval("DATAENTRY.style.display='none'");
hlp = '<%=trim(hlp)%>' 	
