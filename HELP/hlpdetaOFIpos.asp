...
aPos = split(pos,",")
'response.write(ubound(apos))
' response.end
mad = ""
ofi = Request.QueryString("ofi")
cli = Request.QueryString("cli")
est = Request.QueryString("codest")
dim acol(10)
...
if ofi = "AUTO" then
' es insert
fat = "SELECT CODIGO FROM OFIS ORDER BY 1 DESC"
    RS.OPEN fat , cnn
    'response.Write(ofi)
   
    IF RS.RECORDCOUNT <= 0 THEN ofi = "00004000" ELSE ofi = RIGHT("0000000000" + trim(CDBL(RS("CODIGO"))+1),8)
    RS.CLOSE    
     'response.Write("<br>")
     'response.Write(ofi)
     'response.write(fat)
     'response.end
    mad =   " insert into ofis (CODIGO, CODEST,CLIENTE, USUARIO, FECHA, ESTADO, EXP_TEL, EXP_AVI )            " & _
            " VALUES ( '"&ofi&"', '"&EST&"', '"&CLI&"', '"&USUARIO&"', GETDATE(), 'A', '','' ) ;   "   
    FOR T=0 TO 9
        if len(trim(aPos(t))) > 0 then
            mad = mad + " insert into ofi_pos (ofi, po, usuario,fecha, estado) values ( " & _
                        " '"&ofi&"', '"&aPos(t)&"', '"&usuario&"', getdate(), 'A' )     "
            mad = mad + "update pos set ofi = '"&ofi&"' where po = '"&aPos(t)&"' ;"

        end if
    NEXT
else
' UPDATE SOLO AGREGA POS A LA OFI Y SUMA CANTIDADES
response.write("update")
mad = mad + "update ofis set codest = '"&est&"'where codigo = '"&ofi&"'"
    FOR T=0 TO 9
            if len(trim(aPos(t))) > 0 then
                mad = mad + " insert into ofi_pos (ofi, po, usuario,fecha, estado) values ( " & _
                            " '"&ofi&"', '"&aPos(t)&"', '"&usuario&"', getdate(), 'A' )     "
...
mad = mad + " update ofis set cantidad =  (SELECT sum(pp.cantidad) from pos pp where pp.ofi = '"&ofi&"') where codigo = '"&ofi&"'"
response.write("<br>")
response.Write(mad)'
'response.end
...
ss =    " select top 1 isnull(t0,'') as t0, isnull(t1,'') as t1, isnull(t2,'') as t2, isnull(t3 ,'') as t3, isnull(t4,'') as t4, isnull( t5,'')as t5," & _
        " isnull( t6,'')as t6, isnull( t7,'')as t7, isnull( t8,'')as t8, isnull( t9,'') as t9 from [po-deta] where po = '"&apos(0)&"'"
rs.open ss,cnn
rs.movefirst
for v = 0 to 9
...
for t=0 to 9
IF LEN(TRIM(APOS(T))) <=0 THEN EXIT FOR
    ...
    mad = mad + " INSERT INTO DBPROD..COMDETOP (DO_CCODIGO,DO_CGRUPO,DO_CTIPO,DO_CCODCOL1,DO_CDESCOL1,DO_NMERMA1,       " & _
                ...
                " DO_NT7, DO_NT8, DO_NT9 , DO_NT10) VALUES('"&ofi&"','"&grupo&"','C','    ', '"&aPos(t)&"', 0,  " & _
    ...
    cad =   " SELECT		pp.po, PP.color ,	PP.UBI, CAN0, CAN1, CAN2,  fob,     " & _
	        ...
	        "  where pp.po = '"&apos(t)&"'	ORDER BY PP.PO, UBI    ;                "
    rs.open cad, cnn
    newfob = 0
    if rs.recordcount >0 then 
        rs.movefirst
        newfob = replace(rs("fob"), "," , "")
        do while not rs.eof
...
BOSS = "SE HA GENERADO LA OFI --> " + OFI

  MAILS = "EXEC SEND_MAIL_OFI '"&OFI&"', '"&BOSS&"', '"&cli&"', '"&usuario&"'"
    'RESPONSE.WRITE(MAILS)
   CNN.EXECUTE MAILS
%>
<script language="jscript" type="text/jscript">
    cad = "../fichaofi.asp?ofi=" + '<%=ofi%>' + '&cli=' + '<%=Request.QueryString("cli")%>'
 //alert(cad)

   parent.window.opener.window.location.replace(cad)
