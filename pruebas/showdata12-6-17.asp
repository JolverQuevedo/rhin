...
<!--------------------------------------------------------------------->        
    ...
    <table align="center" cellspacing="1" border="1" cellpadding="3" id="resumenS" name="resumenS" style="display:block" >
        <tr><td height="60px;" colspan="16" class="tituloMODELADOR">ORDEN DE  <%=mov %>         </td></tr>
                ...
                <% rs3.open "selEct * from tempo ",cnn
                    IF RS3.RECORDCOUNT > 0 THEN RS3.MOVEFIRST
                    DO WHILE NOT RS3.EOF
                        texto = ucase(RS3.FIELDS.ITEM(0))
                        if inStr(ofis,texto) = 0 then ofis = ofis + trim(texto) + "','" 
                        RS3.MOVENEXT                        
                    LOOP
                ...
                <%RS.CLOSE
                   ...
                   'RESPONSE.END
                    RS.OPEN CAD,CNN%>
                    <tr  class="TEXTO4" id="Tr1">
                        <td ><%if rs.recordcount> 0 then
                                if ucase(trim(ori)) = ucase(TRIM((RS("SK_CALMA")))) and CDBL(RS("SK_NSKDIS")) <> 0  then %>
                                    <input id="Checkbox1" type ="checkbox" style="display:block;">
                                <%end if %>
                            <%end if %>
                        &nbsp;</td>
                        <td style="text-align:left;  background-color:#CFFEDD" id="Td1" ><%=TRIM(ac1(i))%></td>
                        <td style="text-align:left;  background-color:#CFFEDD" id="Td2"><%=TRIM(aD1(i))%></td>
                        <td style="text-align:right; background-color:#CFFEDD" id="Td3"><%=FORMATNUMBER(TRIM(ak1(i)),3,,,TRUE)%>
                        <input id="Text1" value="<%=FORMATNUMBER(TRIM(ak1(i)),3,,,TRUE)%>" style="display:none;width:60px" /></td>
                        <td style="text-align:right; background-color:#CFFEDD" id="Td4"> 
                            <input id="Text2" value="<%=FORMATNUMBER(TRIM(aq2(i)),0,,,TRUE)%>" style="display:none;width:60px" />                
                    ...
                    <% IF RS.RECORDCOUNT > 0 THEN %>  <!-- ES PORQUE ENCONTRO DATOS EN EL ARCHIVO DE STOCKS.... -->
                        <td style="text-align:right; background-color:#FCE0F9" id="Td5"><%=TRIM(RS("UND"))%></td>          
                        <td style="text-align:right; background-color:#FCE0F9;cursor:pointer;" ondblclick="partida('a','<%=i%>')" id="Td6" >
                        ...
                        <td style="text-align:right; background-color:#FCE0F9" id="Td7">
                        <%=TRIM((RS("SK_CALMA")))%></td>            
                        <td style="text-align:right; background-color:#C1D3FF;">
                        <input type="text" value="<%=suge%>" class="DATOSceleste" id="Text3" style="text-align:right; padding-right:10px;width:60px" onchange="valida(this,'<%=i%>')"  onfocus="seleccionar(this)" />
                        <%if suge = "" then suge = 0 %>
                        <input id="Text4" name="su<%=i%>" value='<%=suge%>' style="display:none;width:60px" /> 
                        </td>                          
                        <td style="background-color:#f7b7f6" >
                            <input id="Text5" name="rs<%=i%>" value='<%=CDBL(rs("rsv"))%>' style="display:none;width:60px" /> 
                        ...
                        <td><%if ucase(trim(des)) = ucase(TRIM((RS("SK_CALMA")))) then %>
                            <input id="Checkbox2" type ="checkbox" style="display:block" onclick="bake('<%=i%>', this)" />
                            <%end if %>&nbsp;</td>
                    ...
                    <%else %>
                        ...
                        <td style="text-align:left ; background-color:#FCE0F9" id="Td8">&nbsp;</td>
                        <td style="text-align:right; background-color:#FCE0F9">&nbsp;</td>
                        <td style="text-align:right; background-color:#C1D3FF;" ondblclick="partida('a','<%=i%>')">
                        <input type="text" value="<%=FORMATNUMBER(TRIM(ak2(i)),3,,,TRUE)%>" class="DATOSceleste" id="Text6" style="text-align:right;padding-right:10px;width:60px" 
                ...
                <%IF RS.RECORdCOUNT >1 THEN 
                    ...
                    <tr  class="TEXTO4" id="Tr2">
                            ...
                            <%END IF%>
                            &nbsp;</td>
                            <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="Td9">&nbsp;</td>
                            <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="Td10">&nbsp;</td>
                            <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="Td11">&nbsp;</td>
                            <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="Td12">&nbsp;</td>
                            <td style="text-align:right; background-color:#FCE0F9" ><%=trim(RS("UND")) %></td>
                                ...
                                <%ELSE%>
                                    <%SUGE = CDBL(TRIM(ak1(i)))%>
                            <%END IF%>
                            <td style="text-align:right; background-color:#FCE0F9; cursor:pointer;" id="Td13" ondblclick="partida('old','<%=i%>')" ><%=dis%></td>
                            <td style="text-align:right; background-color:#FCE0F9;" id="Td14"><%=TRIM(RS("SK_CALMA"))%></td>
    ...
    <table align="center" cellspacing="0" border="1" cellpadding="0" id="detaS" name="detaS" style="display:block">
        <tr><td align="center" colspan="<%=columnas+7 %>"  class="tituloGRANDEROJO" 
            ...
            <tr style="padding-right:10px; padding-left:10px;" >
                ...
                <%if lt1 = "h" and LT2= "h"  then %><!-- ORIGEN -->
                    ...
                    <%FOR w=2 TO mani+2%>
                        ...
                        <%END IF%>
                    <%NEXT %>
                     <td style="text-align:right; " class="textogris4"><input id='OC<%=CONTA%>' type="checkbox" onclick="PEDIDO" /></td>
                <%elseif lt1 = "h" then%>
               
                    <%mAC=1%> 
                                  
                    <%FOR w=2 TO mani+3%>
                        <%' IF len(TRIM(RS2.FIELDS.ITEM(w))) > 1Then mac = mac +1%>
                            <td style="text-align:right; " class="textogris4"><%=RS2.FIELDS.ITEM(w)%>&nbsp;</td>
                            <td style="text-align:right; " class="textogris4"><%=formatnumber(RS2.FIELDS.ITEM(w+6),3,,,TRUE)%></td>
                            <%mac=mAC+1%>
                        <%'END IF%>
                    ...
                    <td style="text-align:right; " class="textogris4"><input id='OC<%=CONTA%>' type="checkbox" onclick="PEDIDO" /></td>
                <%else%> 
                    <td style="text-align:right; " class="textogris4"><%=RS2.FIELDS.ITEM(2)%></td>
...
<%ELSE %>
   <!----me chife el cuadro de abajo para ponerlo antes del detalle del servicio de transformacion ---------------------------------------->
<%END IF%>   
</form>  
<script language="jscript" type="text/jscript">
...
function reserva2() {
        ...
        if (rsv == 0) {
            ...
            cad = 'help/hlpRSV.asp?cod=' + aCod + '&alm=' + aAlm + '&can=' + aCan + '&lt2=' + '<%=lt1%>' + '&des=' + aDes
            //    alert('<%=txtusuario%>')
            window.open(cad)
        }
    }
