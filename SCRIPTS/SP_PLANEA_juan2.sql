USE [MODELADOR]
GO
/****** Object:  StoredProcedure [dbo].[SP_PLANEA_juan]    Script Date: 01/09/2017 11:23:17 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[SP_PLANEA_juan2]
(	@cli char(5),
	@FEC VARCHAR(10),
	@GTE varchar(200),
	@POO CHAR(20),
...
IF LEN(LTRIM(RTRIM(@TEL))) = 0 	SET @TEL = NULL
/*
PRINT @CLI
PRINT @FEC
PRINT @GTE
...
INSERT INTO #TMP1 (CLI,NOM,TEMPO,CODTEM, FABRIC,PO,ESTCLI ,OFI,ajuste,COLOR,  LLEGAPO, [ID], CONSUMO, CANPZA, [CODIGO],
[XS], [S], [M] , [L], [XL], [XXL], [3XL],[4XL],[5XL], [6XL],[7XL],[8XL], TOTAL, 
[_XS], [_S],[_M], [_L], [_XL], [_XXL], [_3XL],[_4XL], [_5XL], [_6XL], [_7XL], [_8XL],[_TOTAL]) 
SELECT CLI,NOM,TEMPO,CODTEM, FABRIC,PO,ESTCLI ,OFI,ajuste2,COLOR,  LLEGAPO, [ID], PESO, CANPZA, [CODIGO],
[XS], [S], [M] , [L], [XL], [XXL], [3XL],[4XL],[5XL], [6XL],[7XL],[8XL], TOTAL, 
[_XS], [_S],[_M], [_L], [_XL], [_XXL], [_3XL],[_4XL], [_5XL], [_6XL], [_7XL], [_8XL],
([_XS]+ [_S]+[_M]+[_L]+[_XL]+ [_XXL]+[_3XL]+[_4XL]+[_5XL]+[_6XL]+ [_7XL]+ [_8XL]) 
FROM VIEW_PEDIDO_TELA_DETA 
where  estado = 'A'
and ((@CLI is null) or (CLI  	= LTRIM(RTRIM(@CLI)) ))
...
and ((@PRO is null) or (ajuste	like  '%'+LTRIM(RTRIM(@PRO))+'%'))
and ((@EST is null) or (REPLACE(ESTCLI, ' ' , '')	=  LTRIM(RTRIM(@EST)) ))
and ((@TEL is null) or (REPLACE(codigo, ' ' , '')	=  LTRIM(RTRIM(@tel)) ))
...
WHILE (EXISTS(SELECT PO FROM #TMP1)) -- PRIME WHILE 
	BEGIN
		...
		INSERT INTO #TMP2 (CLI,NOM,TEMPO, CODTEM, FABRIC,PO,ESTCLI ,OFI,ajuste,COLOR,  LLEGAPO, [ID], CONSUMO, CANPZA, [CODIGO],
				[XS], [S], [M] , [L], [XL], [XXL], [3XL],[4XL],[5XL], [6XL],[7XL],[8XL], TOTAL, 
				[_XS], [_S],[_M], [_L], [_XL], [_XXL], [_3XL],[_4XL], [_5XL], [_6XL], [_7XL], [_8XL],[_TOTAL],
				ACOD, ACON, APZA, AT1, AT2, AT3, AT4, AT5, AT6, AT7, AT8, AT9, AT10, AT11, AT12 ) 
				SELECT CLI,NOM,TEMPO, CODTEM, FABRIC,PO,ESTCLI ,OFI,ajuste,COLOR,  LLEGAPO, [ID], CONSUMO, CANPZA, [CODIGO],
				[XS], [S], [M] , [L], [XL], [XXL], [3XL],[4XL],[5XL], [6XL],[7XL],[8XL], TOTAL, 
				[_XS], [_S],[_M], [_L], [_XL], [_XXL], [_3XL],[_4XL], [_5XL], [_6XL], [_7XL], [_8XL],
				...
				ATOC=  CASE WHEN LEFT(ACOD,1)='9' THEN ((AT1+AT2+AT3+AT4+AT5+AT6+AT7+AT8+AT9+AT10+AT11+AT12)*CEILING(@CONS)*@CNPZA) ELSE ACON END
			...
			-- hay que barrer TODAS las telas para ir actualizando los bloques	 ANTES de cambiar de PO					
				...
				if @id <= @maxi
					begin
						...
						((BT1+BT2+BT3+BT4+BT5+BT6+BT7+BT8+BT9+BT10+BT11+BT12)*CEILING(@CONS)*@CNPZA)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
				...
				if @id <= @maxi
					begin
						...
						((CT1+CT2+CT3+CT4+CT5+CT6+CT7+CT8+CT9+CT10+CT11+CT12)*CEILING(@CONS)*@CNPZA)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
				...
				if @id <= @maxi
					begin
						...
						((DT1+DT2+DT3+DT4+DT5+DT6+DT7+DT8+DT9+DT10+DT11+DT12)*CEILING(@CONS)*@CNPZA)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
				...
				if @id <= @maxi
					begin
						...
						((ET1+ET2+ET3+ET4+ET5+ET6+ET7+ET8+ET9+ET10+ET11+ET12)*CEILING(@CONS)*@CNPZA)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
				...
				if @id <= @maxi
					begin
						...
						((FT1+FT2+FT3+FT4+FT5+FT6+FT7+FT8+FT9+FT10+FT11+FT12)*CEILING(@CONS)*@CNPZA)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
						DELETE #TMP1 WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID		
					end
				...
				if @id <= @maxi
					begin
						...
						((HT1+HT2+HT3+HT4+HT5+HT6+HT7+HT8+HT9+HT10+HT11+HT12)*@CNPZA)*CEILING(@CONS)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
						DELETE #TMP1 WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID		
					end
				...
				if @id <= @maxi
					begin
						...
						( (IT1+IT2+IT3+IT4+IT5+IT6+IT7+IT8+IT9+IT10+IT11+IT12)*@CNPZA)*CEILING(@CONS)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
						DELETE #TMP1 WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID		
					end
				...
				if @id <= @maxi
					begin
						...
						( (JT1+JT2+JT3+JT4+JT5+JT6+JT7+JT8+JT9+JT10+JT11+JT12)*@CNPZA)*CEILING(@CONS)
						ELSE @CONS END
						WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID
						DELETE #TMP1 WHERE REPLACE(PO,' ' ,'')= @PO AND ID=@ID		
					end
