...
declare @cli char(5), @id int, @tal varchar(5), @TEL VARCHAR(25), @OFI CHAR(8),@T0 varCHAR(5),@DES VARCHAR(200),@C0 NUMERIC(12,3)
...
ORDER BY Orden
--select * from #tmp2
-- BARRE LA TABLA QYUE TIENE TODAS LAS TALLAS
...
WHERE LEFT(TELA,1) ='9' AND et.OFI in ('00009190','00009203')
...
WHILE (EXISTS(SELECT top 1 OFI,TELA FROM #TMP3 order by Row# )) -- SEGUNDO WHILE 
	BEGIN
		...
		SET @QUERY2 = 'INSERT INTO #TMP1 (CLI, TELA, DESCRI, row#,['+@t0+'])'		
		set @query2 = @query2 + 'VALUES('''+ @cli+''','''+@TEL+''','''+@DES+''','''+@fil+''','
	...
	END -- SEGUNDO WHILE 
set @QUERY = 'select CLI, TELA, DESCRI '
...
set @QUERY = @QUERY + ' from #tmp1 group by cli, tela, descri'
exec(@query)	
