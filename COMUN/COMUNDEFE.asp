...
*/	
	var cad =   'cod=' + trim(thisForm.COD.value)  ;
	    ...
	    cad += '&ABR=' + thisForm.ABR.value;  
	    cad += '&TOL=' + thisForm.TOL.value;  
	    cad += '&LARGO=' + largo    ;
        //alert(chk)
	if (ok == true )
...
{	var pos = parseInt(ff,10)
	...
	thisForm.ABR.value = ltrim(t.rows(pos).cells(3).innerText) ;
	thisForm.TOL.value = ltrim(t.rows(pos).cells(4).innerText) ;
	thisForm.COD.readOnly= true
	oldrow=pos
	return true;
}
...
{	if (trim(thisForm.DES.value) == "" )
	{	alert('Debe Informar la  DESCRIPCION');
		window.thisForm.DES.focus();
		return false;
	}
	if (trim(thisForm.ABR.value) == "" )
	{	alert('Debe Informar la  ABREVIATURA');
		window.thisForm.ABR.focus();
		return false;
	}
		if (trim(thisForm.TOL.value) == "" )
	{	alert('Debe Informar la  TOLERANCIA');
		window.thisForm.TOL.focus();
		return false;
	}
	return true;
}
