...
thisForm.DES.value = '';
thisForm.FAM.disabled=false;
thisForm.FAM.selectedIndex = -1;
}
...
{	var pos = parseInt(ff,10)
	...
	thisForm.COD.value = ltrim(t.rows(pos).cells(2).innerText );
...
{	var t = document.all.TABLA;
	var cod = t.rows(1).cells(0).innerText+t.rows(1).cells(2).innerText; // primer valor del browse
	niv =('<%=nivel%>')
	pagesize = '<%=pagesize%>';
...
{	if (trim(thisForm.DES.value) == "" )
	...
	{	alert('Debe Informar la  FAMILIA');
		window.thisForm.FAM.focus();
		return false;
	}
	return true;
}
