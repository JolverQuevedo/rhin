...
fat = "SELECT CODIGO FROM OFIS ORDER BY 1 DESC"
    ...
    mad =   " insert into ofis (CODIGO, CODEST,CLIENTE, USUARIO, FECHA, ESTADO, EXP_TEL, EXP_AVI )            " & _
            ...
            mad = mad + " insert into ofi_pos (ofi, po, usuario,fecha, estado) values ( " & _
                        " '"&ofi&"', '"&pos&"', '"&usuario&"', getdate(), 'A' )     "
            mad = mad + "update pos set ofi = '"&ofi&"' where po = '"&pos&"' ;"
...
<%	SET RS  = NOTHING
	Cnn.Close
	SET Cnn = NOTHING %>
    <iframe  width="100%" src="" id="body0" name="body0" scrolling="yes" frameborder="1" height="400" align="middle" ></iframe> 


<script language="jscript" type="text/jscript">
    document.all.body0.src = "dbprodOFIpos.asp?ofi=" + '<%=ofi%>' + '&pos=' + '<%=pos %>' + '&cli=' + '<%=cli%>' + '&est=' + '<%=est%>'
</script>
