...
DES = prepara_str_sql(ucase(TRIM(Request.QueryString("DES"))))
MER = ucase(TRIM(Request.QueryString("MER")))
...
IF TOTALREG>0 THEN	
	...
	CAD = 	" insert into "&TBL&" ("&PK&",      " & _
			" "&DS&", TIPO, ESTADO, 	    " & _
			" MERMA, usuario, fecha , UNIDAD, MONEDA, COSTO)                  " & _
			" values('"&COD&"', '" & DES & "',  " & _
			"  '"&TIP&"', 'A', '"&MER&"',			" & _
			" '"&USUARIO&"', "&fecha&", '"&UNI&"', '"&MON&"', "&CTO&")  		" 
End if
