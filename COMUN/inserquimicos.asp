...
top.window.alert("Quimico Duplicado")
...
IF TOTALREG>0 THEN	
	...
	XX = "qm" + TRIM(FAM)+"%"
	CAD = " SELECT COUNT(codigo) FROM quimicos WHERE codigo like '"&XX&"' "
	RESPONSE.WRITE("<br>")
	response.Write(cad)
	rs.open cad,cnn
	...
	COD = "QM" + TRIM(FAM) + RIGHT("00000" + TRIM(CSTR(CORR)),5)
	RESPONSE.Write(COD)
	CAD = 	" insert into "&TBL&"                	" & _
			...
			" "&COSTO&", 'A', '"&USUARIO&"',    	" & _
			" "&fecha&" )  ;							" 
	CAD = CAD + " EXEC RS_AR_I_quimi  '"&COD&"', '"&USUARIO&"'; "
End if
