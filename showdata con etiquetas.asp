                     ...
                     <%RS.CLOSE
                        ...
                        RS.OPEN CAD,CNN%>
                          <tr  class="TEXTO4" id="lin<%=i%>">
                            <td ><input id="ch<%=I%>" type ="checkbox" style="display:block;">&nbsp;ch<%=I%></td>
                            <td style="text-align:left;  background-color:#CFFEDD" id="c<%=i%>" ><%=TRIM(ac2(i))%>-c<%=i%></td>
                            <td style="text-align:left;  background-color:#CFFEDD" id="d<%=i%>"><%=TRIM(aD2(i))%>-d<%=i%></td>
                            <td style="text-align:right; background-color:#CFFEDD" id="k<%=i%>"><%=FORMATNUMBER(TRIM(ak2(i)),3,,,TRUE)%>-k<%=i%>
                            <input id="kg<%=i%>" value="<%=FORMATNUMBER(TRIM(ak2(i)),3,,,TRUE)%>" style="display:block;width:60px" /></td>
                            <td style="text-align:right; background-color:#CFFEDD" id="q<%=i%>"> 
                              <input id="qt<%=i%>" value="<%=FORMATNUMBER(TRIM(aq2(i)),0,,,TRUE)%>" style="display:block;width:60px" />                
                        ...
                        <% IF RS.RECORDCOUNT > 0 THEN %>  <!-- ES PORQUE ENCONTRO DATOS EN EL ARCHIVO DE STOCKS.... -->
                            ...
                            <%if UCASE(trim(RS("UND"))) = "UND" THEN 
                                response.write(FORMATNUMBER(TRIM(CDBL(RS("SK_NSKDIS"))),0,,,TRUE)) 
                              else 
                                response.write(FORMATNUMBER(TRIM(CDBL(RS("SK_NSKDIS"))),3,,,TRUE))
                              end if%>
                            ...
                            <td style="text-align:right; background-color:#FCE0F9" id="a<%=i%>"><%=TRIM((RS("SK_CALMA")))%>-a<%=i%></td>            
                            ...
                            <%if suge = "" then suge = 0 %>
                            <input id="su<%=i%>" name="su<%=i%>" value='<%=suge%>' style="display:block;width:60px" /> -sug<%=i%>
                            </td>                          
                            <td style="background-color:#f7b7f6" >
                             <input id="rs<%=i%>" name="rs<%=i%>" value='<%=CDBL(rs("rsv"))%>' style="display:block;width:60px" />
                            <%IF CDBL(rs("rsv")) > 0 THEN %><%=FORMATNUMBER(CDBL(rs("rsv")),3,,,true)%><%else%>&nbsp;<%end if%>-rs<%=i%> </td>                           
                            <script language="javascript" type="text/jscript">
                               // modifica el sugerido!!!
                               i = parseInt('<%=i%>', 10)
                               dd = i.toString()
                               suger = parseFloat(document.getElementById("su" + dd).value);
                               kilos = parseFloat(document.getElementById("kg" + dd).value);
                               canti = parseFloat(document.getElementById("qt" + dd).value);
                               reser = parseFloat(document.getElementById("rs" + dd).value);
                               nuevo = '';
                               if (reser > 0) {
                                   if (canti > 0) {
                                       if (parseInt(canti, 10) > parseInt(reser, 10))
                                       { nuevo = parseFloat(suger) - parseFloat(reser) }
                                   }
                                   else {
                            ...
                            <td><input id="co<%=i%>" type ="checkbox" style="display:block" onclick="bake('<%=i%>', this)" />&nbsp;-co<%=i%></td>
                        ...
                        <%else %>
                            ...
                            <td style="text-align:left ; background-color:#FCE0F9" id="s<%=i%>">&nbsp;-s<%=i%></td>
                            <td style="text-align:right; background-color:#FCE0F9">&nbsp;</td>
                            <td style="text-align:right; background-color:#C1D3FF;" ondblclick="partida('a','<%=i%>')">
                            <input type="text" value="<%=FORMATNUMBER(TRIM(ak2(i)),3,,,TRUE)%>" class="DATOSceleste" id="sug<%=i%>" style="text-align:right;padding-right:20px;" 
                             onchange="valida(this,'<%=i%>')"  onfocus="seleccionar(this)"/>-sug<%=i%></td>
                             <td style="background-color:#f7b7f6">&nbsp;</td>
                            <td><input id="co<%=i%>" type ="checkbox">&nbsp;-co<%=i%></td>
                        ...
                        <%IF RS.RECORdCOUNT >1 THEN 
                           ...
                           <tr  class="TEXTO4" id="ln<%=i%>">
                                <td><input id="cc<%=i%>" type ="checkbox" />&nbsp;-cc<%=i%></td>
                                <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="ca<%=i%>">&nbsp;-ca<%=i%></td>
                                <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="da<%=i%>">&nbsp;-da<%=i%></td>
                                <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="ka<%=i%>">&nbsp;-ka<%=i%></td>
                                <td style="text-align:left;  background-color:#CFFEDD; color:green;" id="qa<%=i%>">&nbsp;-qa<%=i%></td>
                                <td style="text-align:right; background-color:#FCE0F9" ><%=trim(RS("UND")) %></td>
                                 ...
                                 <%if UCASE(trim(RS("UND"))) = "UND" THEN%>
                                    ...
                                    <%ELSE%>
                                        <%SUGE = CDBL(TRIM(ak2(i)))%>
                                <%END IF%>
                                <td style="text-align:right; background-color:#FCE0F9; cursor:pointer;" id="st<%=i%>" ondblclick="partida('old','<%=i%>')" ><%=dis%> -st<%=i%></td>
                                <td style="text-align:right; background-color:#FCE0F9;" id="old<%=i%>"><%=TRIM(RS("SK_CALMA"))%>-old<%=i%></td>
                                <td style="text-align:right; background-color:#C1D3FF" >&nbsp;</td>
                                 <td style="background-color:#f7b7f6">&nbsp;</td>
                                <td><input id="coo<%=i%>" type ="checkbox">&nbsp;-coo<%=i%></td>
...
<script language="jscript" type="text/jscript">
    ...
    var des = trim('<%=des%>')
    for (v = 0; v < maxi; v++) {
        if (trim(eval("document.all.s" + v + ".innerText")) == '') {
            eval("document.all.ch" + v + ".style.display='none'")
            eval("document.all.co" + v + ".style.display='none'")
        }
       
        dd = "st" + v.toString()
        var ele = document.getElementById(dd);
        if (ele !== null) {
            if (trim(eval("document.all.st" + v + ".innerText")) == '' || trim(eval("document.all.ca" + v + ".innerText")) == '') {
                eval("document.all.cc" + v + ".style.display='none'")
                eval("document.all.coo" + v + ".style.display='none'")
            }
        }
    }
 /*   for (t = 0; t < maxi; t++) {
        if ( eval("document.all.co" + t + ".style.display") == 'none') {
            eval("document.all.ch" + t + ".style.display='none'")
        }
    ...
    */
    ...
    for (q = 0; q < maxi; q++) {
    if (eval("document.all.co" + q + ".style.display") == 'block') 
    {  eval("document.all.co" + q + ".style.backgroundColor='#ff0000'")                
    }
    }
    */   
...
function bake(pos, obj) {
    ...
    if (obj.checked == true) {
        ...
        cad = 'bake/bakeOC.asp?pos=' + pos + '&ofi=' + ofi + '&cod=' + cod + '&alm=' + alm
