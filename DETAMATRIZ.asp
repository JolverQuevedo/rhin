...
{   var cad  = "SEARCH/buscaDOR.asp?pos="+color
        ...
        cad += '&col=CODIGO'
        cad += '&col2=cliente'
        cad += '&msg=NO EXISTEN ESTAMPADOS PARA ESTE CLIENTE'
        cad += '&pos2=' + '<%=trim(request.QueryString("CLI"))%>'
        cad += '&url=../detaMATRIZ.asp?cli=' + '<%=trim(request.QueryString("CLI"))%>'
    document.all.bake.src=cad
}
function fReporte()
{	window.open('REPORTES/prnTBL.asp?CLI=<%=TRIM(REQUEST.QUERYSTRING("CLI"))%>')
}
var oldrow=1
</script>
...
tbl = trim(application("owner"))+".MATRIZ"
tabla = ucase("MATRIZ")
