...
<tr>
    ...
    <tr> 
		...
		<td width="10%">
		    <input name="ART" type="TEXT" class="DATOSGRANDE" id ="ART" maxlength="20" width="100" value="<%=request.querystring("art")%>" <%if ucase(trim(txtusuario)) <> "SISTEMAS" THEN %> READONLY <%END IF%>/></td>
 ...
 <!--   <tr>    
    ...
    -->
        <td colspan ="7" align="center"> <input type="button" id="manda" onclick="VERI()" value="GRABAR" /></td>
	</tr>
<!--	<%IF  REQUEST.QUERYSTRING("cod") <> "AUTO" THEN %>	
	<tr>
	    <td colspan ="7" align="center" width="100%">
	    <img src="bake/RECUPERAFOTOESTILOr.ASP?CLI=<%=CLIENTE%>&ID=<%=REQUEST.QUERYSTRING("cod")%>" width="500" /></td>	    
    </tr>
    <%END IF %>
    -->
  </table>	
...
function VERI() {
    ...
    document.all.formato.action = 'comun/InserfichaESTR.asp'  
