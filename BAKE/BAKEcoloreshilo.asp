...
CAD =   " select * from (SELECT C1.REALSYS as colo, C2.DESCRIPCION AS DESCRI            " & _
		" FROM RAYADOS AS C1                                                            " & _
		" INNER JOIN COLORES AS C2 ON C1.COLOR = C2.COdigo                              " & _
        " WHERE C2.CLIENTE = '"&DATO&"'  and C1.estado = 'A' AND ('"&TELA&"'+C1.REALSYS " & _
        " NOT IN (select ar_ccodigo COLLATE Modern_Spanish_CI_AS                        " & _
        " from RSFACCAR..AL0001ARTI))) dd  ORDER BY 1                                   " 
