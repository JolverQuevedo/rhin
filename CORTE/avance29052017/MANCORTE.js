			...
			if($.trim(data[0].CT_CNROCORTE)!='')
				{
					...
					//alert(data[0].CT_CDESSER);
					//$('#txtModulo').val(data[0].CT_CDESSER);
					var i = 1;
					$("#cboModulo option").each(function(){
						
						//alert(_value);
						 _value = $.trim(data[0].CT_CDESSER);
						 //_value2 = $.trim($('#txtModulo').val());
						 _value2 = $.trim($(this).text());
						 //alert(i);
						 ...
						 if(_value == _value2)
						 {
							...
							//$(this).val();
							//$('#cboModulo option[value=' + $(this).val() + '"]').attr("selected", "selected");
							$("#cboModulo option[value="+ $(this).val() +"]").attr("selected",true);
						 }
						// else{
							 //
						 //}
...
$('#tbColores tr:last').after('<tr class="fila" id="fila'+x+'"><td><input type="checkbox" id="'+d.do_ccodcol1+'" /></td><td id="f'+$.trim(d.do_ccodcol1)+'">'+$.trim(d.do_cdescol1)+'</td></tr>');	
	...
	$('#bntGrabar').click(function(){
		...
		if($.trim($('#txtnroCORTE').val())=='')
			{
				alert('Debe ingresar N° de Corte!!');
				return;
			}
		...
		if(confirm('¿Seguro de modificar Corte?'))
			{
					...
					//window.open('http://localhost/modelador/webservice/GrabaManCorteFox.asp?numCorte='+numCorte+'&fecha='+fecha+'&CPO='+CPO+'&costoEst='+costoEst+'&costoReal='+costoReal+'&kilos='+kilos+'&sobra='+sobra+'&tapeta='+tapeta+'&ancho='+ancho+'&largo='+largo+'&metro='+metro+'&punta='+punta+'&retazo='+retazo+'&falla='+falla+'&pepan='+pepan+'&pazpano='+pazpano+'&nropano='+nropano+'&partida='+partida+'&codTela='+codTela+'&nropaquete='+nropaquete+'&codservicio='+codservicio+'&servicio='+servicio);
					$.getJSON('http://localhost/modelador/webservice/GrabaManCorteFox.asp?numCorte='+numCorte+'&fecha='+fecha+'&CPO='+CPO+'&costoEst='+costoEst+'&costoReal='+costoReal+'&kilos='+kilos+'&sobra='+sobra+'&tapeta='+tapeta+'&ancho='+ancho+'&largo='+largo+'&metro='+metro+'&punta='+punta+'&retazo='+retazo+'&falla='+falla+'&pepan='+pepan+'&pazpano='+pazpano+'&nropano='+nropano+'&partida='+partida+'&codTela='+codTela+'&nropaquete='+nropaquete+'&codservicio='+codservicio+'&servicio='+servicio,function(data){
						...
						if(data.status==1)
							...
							for(i=1;i<=$('#tbTallas tr').length-2;i++)
								{ 	
									...
									CD_CNROOFI 		= $('#celOFI').html();
									CD_CNROCORTE 	= data.nrocorte;
									CD_CSECUEN 		= $('#cNum'+i).html();
									CD_CARTICULO    = $('#cArt'+i).html();
									CD_CTALLA 		= $('#cTal'+i).html();
									CD_FECHA		= $('#txtFecha').val();
									//CD_CCODCOL 		= _color[0];
									//CD_CCOLOR 		= _color[1];
									CD_NCANT		= $('#txtCan'+i).val();
									CD_NINI			= $('#cDes'+i).html();
									...
									//window.open('http://localhost/modelador/webservice/GrabaManCorteDetFox.asp?CD_CNROOFI='+CD_CNROOFI+'&CD_CNROCORTE='+CD_CNROCORTE+'&CD_CSECUEN='+CD_CSECUEN+'&CD_CARTICULO='+CD_CARTICULO+'&CD_CTALLA='+CD_CTALLA+'&CD_NCANT='+CD_NCANT+'&CD_NINI='+CD_NINI+'&CD_NFIN='+CD_NFIN+'&CD_NORDEN='+CD_NORDEN+'&tabs='+_x+'&item='+i+'&paqt='+nropaquete);
									$.getJSON('http://localhost/modelador/webservice/GrabaManCorteDetFox.asp?CD_CNROOFI='+CD_CNROOFI+'&CD_CNROCORTE='+CD_CNROCORTE+'&Fecha='+CD_FECHA+'&CD_CSECUEN='+CD_CSECUEN+'&CD_CARTICULO='+CD_CARTICULO+'&CD_CTALLA='+CD_CTALLA+'&CD_NCANT='+CD_NCANT+'&CD_NINI='+CD_NINI+'&CD_NFIN='+CD_NFIN+'&CD_NORDEN='+CD_NORDEN+'&tabs='+_x+'&item='+i+'&paqt='+nropaquete,function(data){
									})
									.done(function(data)
										{
										if(data.status==1)
