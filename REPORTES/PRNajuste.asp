...
var aMat = new Array()
	...
	CAD =	" exec FICHAajuste '"&AJU&"' , '"&VER&"' , '00001' " 
	...
	IF RS("COTI")= "AUTO" THEN
	    marka = 0	       
		titulo = "HOJA DE AJUSTE #"
	else
	    marka = 1
		titulo = "HOJA DE AJUSTE # " + AJU + " - " + VER
	END IF%>
<html xmlns="http://www.w3.org/1999/xhtml" lang="es" >
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" lang="es-pe" />
<title>FICHA AJUSTE</title>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr valign="center" > 
          ...
          <table cellpadding="0" cellspacing="0" border="0" align="center" width="100%">
                ...
                <tr><td align="center" class="texto "><input id="FECCOT" name="FECCOT" value='<%=left(RS("FECHA"),10)%>' class="cambio" readonly tabindex="-1" ></td></tr>
		    </table> 
          </td>		
...
<table width="100%" border="1" cellspacing="1" cellpadding="1">
  <tr>
        <td width="5%"  class="american2">AJUSTE :</td>
        <td width="15%"  class="TEXTO"><%=rs("AJU")%>-<%=rs("ver")%></td>
    ...
    <tr>
        ...
        <td width="10%" class="TEXTO" ><%=trim(rs("mot"))%></td>
    </tr>
  <tr>
    <td width="5%"  class="american2">TELA</td>
    <td width="80%" class="TEXTO" colspan="7">
  ...
  <tr>
   <%DAD = "SELECT TALLA,PROP, cant, orden FROM [AJUSTE-TALLAS] WHERE AJUSTE = '"&AJU&"' AND VERSION = '"&VER&"' ORDER BY ORDEN"
  ' RESPONSE.WRITE(DAD)
   RS2.OPEN DAD, CNN 
    ...
    <table  border="1" cellspacing="1" cellpadding="0" align="left" width="100%">
        <tr>
            <td  class="american2">  TALLAS :      </td>
            <%FOR I = 0 TO 9%>
               <td width="8%"  class="american2">
                <%if i <= UBOUND(ATAL) then  %>
              <%=trim(atal(i))%>
               <%else %>
                <input id="Text2" name="T<%=i%>" readonly tabindex="-1" class="TEXTOblue" style="width:30px;text-align:center">   
               <%end if %>
               </td>    
            <%NEXT%>
...
<table border="0" cellpadding="0" width="100%" cellspacing="0" bgcolor="<%=(Application("color2"))%>" bordercolordark="<%=(Application("BORDE"))%>" bordercolorlight="<%=(Application("COLOR2"))%>" id="TELAS">
	...
	<%DO WHILE NOT RS2.EOF%>
		<tr valign="top"  bgcolor="<%=(Application("color1"))%>" > 
         <td width="1%"><input  type="text"  id="ID1<%=i%>" name="ID1<%=i%>" value="<%=trim(rs2("id"))%>" class="barras" readonly tabindex="-1" /></td>      
            <td width="4%"  align="center"><input id="TEL1<%=i%>" name="TEL1<%=i%>" value="<%=trim(rs2("TEL"))%>"   type="text"  class="barras" readonly tabindex="-1" /></td>
            <td width="50%" class="barras"><%=trim(rs2("DTE")) %> - <%=trim(rs2("com")) %></td> 
            <td width="3%"><input type="text" id="ANC<%=i%>" name="ANC<%=i%>" value="<%=formatnumber(rs2("ANC"),2,,,true)%>"class="BARRAS" readonly tabindex="-1" style="width:100%;text-align:left; vertical-align:top" /></td>
            <td width="3%" ><input type="text" id="LAR<%=i%>" name="LAR<%=i%>" value="<%=formatnumber(rs2("LAR"),2,,,true)%>" class="BARRAS" readonly tabindex="-1" style="width:100%;text-align:left; vertical-align:top" /></td>
            <td width="5%"><input type="text" id="CUN<%=i%>" name="CUN<%=i%>" value="<%=formatnumber(rs2("CUN"),3,,,true)%>" class="BARRAS" readonly tabindex="-1" style="width:100%;text-align:left; vertical-align:top" /></td>
            <td width="3%"><input type="text" id="MERI<%=i%>" name="MERI<%=i%>" value="<%=trim(rs2("MER"))%>" class="BARRAS" readonly tabindex="-1" style="width:100%;text-align:center; vertical-align:top" /></td>
                ...
                <tr valign="top">
                                    ...
                                    <tr>
                                     ...
                                     <script language="jscript" type="text/jscript">
                                           ...
                                           if (parseFloat(cc) <= 0)
                                                cc = 0.01
                                           pda = tt * cc
                                           eval("document.all.HH" + x + ".value=deci(pda,3)")
                    ...
                    <td width="50%">
                        <table width="100%" border="0" cellpadding="1" cellspacing="0">
                            ...
                            <%cad = "EXEC FICHACOTI_RUTA '"&COT&"' , '"&TEL&"' , '"&RUT&"', "&ID&" , '"&AJU&"',  '"&VER&"' "    
                                   'RESPONSE.Write(CAD)
                            RS3.OPEN CAD,CNN%>
                            ...
                            <%IF RS3.RECORDCOUNT>0 THEN
                                ...
                                <tr>
                                    ...
                                    <td width="7%"><input id="SER<%=k%>" name="SER<%=k%>" value="<%=UCASE(TRIM(RS3("codigo")))%>" class="barras" readonly tabindex="-1" /></td>
                                    ...
                                    <td><input type="text" id="M<%=k%>" name="M<%=k%>" value="<%=TRIM(RS3("MERMA"))%>" class="barras" readonly tabindex="-1" style="text-align:center" /></td>
	...
	<%DO WHILE NOT RS2.EOF%>
        <tr valign="top"> 
         <td><input id="AID<%=XON%>" name="AID<%=XON%>" value="<%=RS2("ORD")%>" readonly tabindex="-1" class="TEXTONAVYREADONLY" /></td>       
           ...
           <td><input type="text" id="CON<%=xon%>" name="CON<%=xon%>" value="<%=formatnumber(cdbl(rs2("CON")),3,,,true)%>" class="TEXTOblue" style="text-align:right; padding-right:10px; size:20px" readonly tabindex="-1" size="5" /></td>
