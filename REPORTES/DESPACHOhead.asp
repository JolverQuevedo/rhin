...
function MUESTRA(op) {
    ...
    datos = trim(thisForm.CLI.value) + trim(thisForm.final.value) + trim(thisForm.EST.value) + trim(thisForm.PO.value) + trim(thisForm.TEM.value) + trim(thisForm.gtex.value)

    if (trim(datos) == '') {
        alert("Seleccione algun criterio por favor")
        return false;
    }
  cad  = 'DESPACHOdeta.asp?cli=' +trim(thisForm.CLI.value)
...
<table id="Table1" align="center"  bordercolor="#FFFFFF"  bgcolor="<%=Application("color2")%>"  cellpadding="1"  cellspacing="1"  border="0">
    <tr valign="middle" >
    <td align="left" rowspan="3" width="10%"><img src="../imagenes/ATRAS.gif" 
      ...
      <% CAD =   " SELECT CODIGO,  NOMBRE FROM CLIENTE WHERE ESTADO ='A' order by NOMBRE "
        ...
        <%RS.CLOSE %>
         ...
         <td width="20%"><input id="EST" name="EST"  value =""  width=70 class="DATOSkhaki"></td>	
     ...
     <tr>
        ...
        <td><input id="PO" name="PO"  value =""  width=70 class="DATOSkhaki"></td>
        <td></td>
    </tr>
    <tr>    	
        ...
        <td><input id="gtex" name="gtex"  value =""  width=70 class="DATOSkhaki"></td>       
