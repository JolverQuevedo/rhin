...
<script type="text/jscript" language="jscript">
    ...
    var marca = 0
    var oldrow = 1
    var url = '../DETAAJUSTE.asp?'
    var alias = 'VIEW_AJUSTES'
    var TBL = 'AJUSTES'
    var PK = 'AJUSTE'
    var DS = 'version'
...
<!--#include file="COMUN/COMUNAJUSTEdeta.asp"-->
...
if aju = "" or isnull(aju)  or aju = " " then
	aju = ""
end if
...
CAD =	" SELECT top 16 AJU, VER, NOM, ECL, DES,             " & _
        " DGEN, MOT AS GPO_TEL, TEL, DTEL, DTEM, ADIC, TPRE, " & _
        " CLI, EST, TEM, ART, TIP, GEN                       " & _
        " FROM VIEW_AJUSTES AS T1                            " & _
		" WHERE T1.EDO = 'A' and                             " & _
        " CLI  = '"&POS&"' and                               "
IF   LEN(DATO) > 0  THEN
    cad = cad + " AJU+ver >= '"&dato&"'	ORDER BY AJU, VER        " 
ELSEIF LEN(TEM) > 0  THEN
    cad = cad + " DTEM >= '"&TEM&"'	 ORDER BY DTEM, AJU, VER     " 
ELSEIF LEN(EST)> 0 THEN
    cad = cad + " ECL >= '"&EST&"'	ORDER BY ECL, AJU, VER       " 
ELSE
    cad = cad + " AJU+ver >= '"&dato&"'	ORDER BY AJU, VER        " 
END IF
...
<form name="thisForm" method="post" action="DETAAJUSTE.asp">
<%'*********************************************************************%>
...
function BUSQUE() {
    ...
    if (pr.length < 14 && pr.length > 0 ) {
        po = pr.indexOf('-')
        if (po <= 0) {
            alert("Informar Ajuste-Versión por favor")
            document.all.thisForm.kd.focus()
            return true
        }
        ppo = Left(pr,po)
        ppo =strzero(ppo,10)
        ppr = trim(document.all.thisForm.kd.value)
        //alert(ppr.indexOf('-'))
        vee = ppr.substring(ppr.indexOf('-') + 1);
       // alert(vee)
       // ver = invertircadena(vee)
        //document.all.thisForm.kd.value = ppo+'-'+ver
    }
    
    cad  = 'DETAAJUSTE.asp?pos=' + '<%=trim(pos) %>'
    cad += '&aju=' + ppo;
    cad += '&ver=' + vee;
...
{ var t = document.all.TABLA;
    ...
    AJU = ltrim(t.rows(pos).cells(0).innerText);
	...
	datos  = '&AJU='+AJU+'&VER='+VER+'&DES='+DES
	datos += '&EST='+EST+'&TEL='+TEL
	datos += '&TEM=' + TEM
	cad = 'fichaAJUSTE.asp?cli=' + '<%=pos%>' + datos
  //  alert(cad)
	window.open(cad)
    return true;
}
...
function IMPRIME() {
    ...
    AJU = trim(t.rows(pos).cells(0).innerText);
    VER = trim(t.rows(pos).cells(1).innerText);
    CAD = 'REPORTES/PRNajuste.ASP?AJU=' + AJU + '&VER=' + VER
...
{	var t = document.all.TABLA;
	...
	CAD += '&pk1=PROTO&PK2=VERSION&tbl=AJUSTES'
	CAD += '&des=' + VER
	CAD += '&URL=../DETAAJUSTE.ASP?POS=' + '<%=Request.QueryString("pos")%>'
    //alert(CAD)
	window.location.replace(CAD); 
	return true;
}
...
//pagina de inicio
pag = 'DETAAJUSTE.asp?cli=' + '<%=trim(request.QueryString("pos"))%>'
</script>    
<%IF txtPerfil = 1 THEN%>
<SCRIPT> 
if (marca == 1)
